...
void GPIO_init(void)
{
	...
	/*PortA (updated): 0x4470555 <= changing to ADC (digital Brake: 0x0445055)
		Assignment:	00	    01       00      01        X00       01     
		Pin#:				12   	  11  	   10       9        X8        7      
								Crs_in  Crs_out  haz_in   MC_out   Xbrk_in   Brk_c   
			
		Assignment:	11    	00      00      01      01      01     01
		Pin#:				6    		5       4       3       2       1      0
								brk_in	RT_in   LT_in   BR_C    BL_C    FR_C   FL_C		*/
	GPIOA->MODER = 0x0445055 ;									
	GPIOA->PUPDR = 0;  							
	//GPIOA->BSRR = MC_Contact_out;		//all outputs except MC are low to start
	GPIOA->OTYPER = 0;
	GPIOA->OSPEEDR = 0x15555555;
	/*
	PortB (updated): =0x1000
	01    	       00              00        00        00   00   00
	6              5               4         3         2    1    0
	Headlights_C   Headlights_in   Regen_in  Accel_in  /    /    /  
	*/
	GPIOB->MODER = 0x1000;						
	GPIOB->PUPDR = 0;  				
	GPIOB->OTYPER = 0;
...
void Headlights(void)
{
	int headlights_on = GPIOB->IDR & Headlights_In;
	if (headlights_on == Headlights_In) 
		{
			//set brakes and back right high 
			GPIOB->BSRR = Headlights_C;
		}
	else
		GPIOB->BRR = Headlights_C;
}
